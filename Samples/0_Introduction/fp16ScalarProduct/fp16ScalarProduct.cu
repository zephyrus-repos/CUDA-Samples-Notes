...
/**
 * @brief 利用半精度浮点向量（half2）指令，高效地计算 128 个元素的总和
 */
...
/**
 * @brief 利用重载的半精度浮点向量加法，高效地计算 128 个元素的总和
 */
...
__global__ void scalarProductKernel_intrinsics(half2 const *const a,
                                               ...
                                               float *const results,
                                               size_t const size) {
    const int stride = gridDim.x * blockDim.x; // 计算线程总数
    __shared__ half2 shArray[NUM_OF_THREADS];  // 共享内存
